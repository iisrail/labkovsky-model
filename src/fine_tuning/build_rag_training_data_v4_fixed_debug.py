...
# Debug controls: set DEBUG_QA_ID / DEBUG_GOLD_TEXT to inspect one case.
DEBUG_QA_ID = "srJvn19GKNA_01"
DEBUG_GOLD_TEXT = "В основе многих зависимостей лежит тревога"
DEBUG_TOP_N = 15
...
def debug_rank_cards_for_qa(
    ...
    """Print exact/alias candidate counts and raw cosine ranks before MMR."""
    ...
    for idx in candidate_indices:
        ...
        scored.append((idx, sim, cards[idx]))
    ...
    for gold_idx in gold_indices:
        ...
        rank = next((pos + 1 for pos, (idx, _, _) in enumerate(scored) if idx == gold_idx), None)
        sim = next((score for idx, score, _ in scored if idx == gold_idx), None)
        print("\nGold card status:")
        ...
        print(f"  raw_rank={rank}, raw_similarity={None if sim is None else round(float(sim), 4)}")

    print(f"\nRAW TOP {DEBUG_TOP_N} BEFORE MMR:")
    for rank, (idx, sim, card) in enumerate(scored[:DEBUG_TOP_N], 1):
        text = card.get("core_principle", "")
        marker = "  <-- GOLD" if idx in gold_indices else ""
        print(f"{rank:02d}. card_{idx} sim={sim:.4f} dts={sorted(get_card_dts(card))}{marker}")
        print(f"    {text[:180]}")
...
def debug_selected_cards(rec_id: str, selected_cards: List[Dict[str, Any]], gold_indices: List[int]) -> None:
    """Print final cards after MMR for one debug QA."""
    if rec_id != DEBUG_QA_ID:
        return
    ...
    for rank, item in enumerate(selected_cards, 1):
        ...
        print(
            ...
            f"mmr={float(item.get('mmr_score', 0)):.4f}" + marker
        )
