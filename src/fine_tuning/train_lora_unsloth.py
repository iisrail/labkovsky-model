...
def load_all_data(input_files: Dict[str, Path]) -> List[dict]:
    ...
    for src, fp in input_files.items():
        ...
        for obj in iter_jsonl(fp):
            # qa_rs_final.jsonl: {"question": ..., "answer_segmented": ...}
...
def split_by_unique_answers(records: List[dict], val_ratio: float, seed: int):
    """
    Split by UNIQUE ANSWERS to prevent data leakage.
    ...
    print(f"  Unique answers: {len(groups)}")
    print(f"  Train groups: {len(train_groups)} -> {len(train_records)} records")
    print(f"  Eval groups: {len(eval_groups)} -> {len(eval_records)} records")
