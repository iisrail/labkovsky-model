...
# -*- coding: utf-8 -*-
"""
train_qa_special_tokens.py

QA LoRA training on merged base with dedicated RS special tokens.
Adds [ОБЪЯСНЕНИЕ], [ВМЕШАТЕЛЬСТВО], [ЭСКАЛАЦИЯ] as single-token specials.
Includes embed_tokens and lm_head in LoRA targets to train the new embeddings.

Usage (WSL):
    source venv_wsl/bin/activate
    python src/fine_tuning/train_qa_special_tokens.py
"""
...
OUTPUT_DIR = PROJECT_ROOT / "models" / "labkovsky-merged-special-tokens"

# QA data + anti_generic with RS tags
INPUT_FILES = {
    "qa_final": FINE_TUNING_DIR / "qa_rs_final.jsonl",
    "anti_generic": FINE_TUNING_DIR / "anti_generic_tagged.jsonl",
...
# RS special tokens
RS_SPECIAL_TOKENS = ["[ОБЪЯСНЕНИЕ]", "[ВМЕШАТЕЛЬСТВО]", "[ЭСКАЛАЦИЯ]"]

# ALL 7 modules (attention + MLP), embeddings trained directly (not via LoRA)
LORA_R = 8
...
def main():
    print("=" * 60)
    print("Labkovsky Fine-tuning (SPECIAL TOKENS + ALL MODULES)")
    print("Base: vikhr-book-merged (has book vocabulary)")
    print("Special tokens: [ОБЪЯСНЕНИЕ], [ВМЕШАТЕЛЬСТВО], [ЭСКАЛАЦИЯ]")
    print("Modules: all 7 + direct embedding grad for new tokens")
    ...
    # Add RS special tokens
    print(f"\nAdding special tokens: {RS_SPECIAL_TOKENS}")
    num_added = tokenizer.add_tokens(RS_SPECIAL_TOKENS, special_tokens=True)
    ...
    for t in RS_SPECIAL_TOKENS:
        ids = tokenizer.encode(t, add_special_tokens=False)
        print(f"  {t} -> {ids} (should be single token)")

    # ALL 7 MODULES LoRA (no embed/lm_head — those are trained directly)
    ...
    # Train only the 3 new token embeddings directly (not via LoRA)
    print("\nEnabling gradients for new token embeddings only...")
    model.get_input_embeddings().weight.requires_grad_(False)
    for i in range(len(RS_SPECIAL_TOKENS)):
        model.get_input_embeddings().weight[-len(RS_SPECIAL_TOKENS) + i].requires_grad = True

    model.get_output_embeddings().weight.requires_grad_(False)
    for i in range(len(RS_SPECIAL_TOKENS)):
        model.get_output_embeddings().weight[-len(RS_SPECIAL_TOKENS) + i].requires_grad = True

    print(f"  Input embeddings: last {len(RS_SPECIAL_TOKENS)} rows have grad=True")
    print(f"  Output embeddings: last {len(RS_SPECIAL_TOKENS)} rows have grad=True")
    ...
    print(f"  Special tokens: {RS_SPECIAL_TOKENS}")
    print(f"  LoRA: r={LORA_R}, alpha={LORA_ALPHA}")
    print(f"  Modules: {TARGET_MODULES} (ALL 7 + new token embeds)")
